...
# 是否对粗选区域做平面精提取（用于去除杂点）
# True  — 调用 detect_planar_patches 精提取，适合平整标定板
# False — 跳过平面检测，直接把粗选 OBB 内的点压平使用，适合微弯标定板
ENABLE_PLANAR_PATCH_EXTRACTION = False

# 提取平面
def extract_plate(pcd: geometry.PointCloud, obb: geometry.OrientedBoundingBox):
    # 先扩展 OBB，确保包含所有点
    indices, expanded_obb = _expand_obb(pcd, obb)
    # 把扩展后的点取出
    selected_pcd = pcd.select_by_index(indices)

    if ENABLE_PLANAR_PATCH_EXTRACTION:
        # 对整体点云做平面精提取，不按颜色聚类
        # （颜色聚类会把棋盘格黑白格拆成两个不连续子云，
        #   导致 detect_planar_patches 检测到的 OBB 只覆盖半张板）
        cropped_pcd, obb = _extract_plate_by_pcd(selected_pcd)
    else:
        # 跳过平面检测，直接用粗选点
        # expanded_obb.R 从粗选 OBB 继承，保留板子真实朝向，aligner 可以正确旋转到 XOY
        logger.info("Planar patch extraction disabled, using expanded OBB points directly.")
        cropped_pcd = selected_pcd
        obb = expanded_obb
...
# 对整体点云做平面精提取
def _extract_plate_by_pcd(pcd: geometry.PointCloud):
    pcd.estimate_normals(search_param=geometry.KDTreeSearchParamKNN(knn=30))
    patches = pcd.detect_planar_patches(normal_variance_threshold_deg=20,
                                        coplanarity_deg=10,
                                        outlier_ratio=0.2,
                                        min_plane_edge_length=0,
                                        min_num_points=0)
    # 找到最大的 OBB（避免与标定板相同材质的杂质干扰）
    biggest_obb, _ = _find_biggest_obb(list(patches))
...
def remove_boundary(pcd, pre_remove_percent=0.02):
    ...
    # 用各点到平面中心（原点）的距离聚类
    # align() 已将板子中心移到原点，边框必然在外侧，内部在中心，与材质特性无关
    points_xy = np.asarray(bound_removed_pcd.points)[:, :2]
    distances = np.linalg.norm(points_xy, axis=1).reshape(-1, 1)
    clustered_pcds = _cluster_pcd(bound_removed_pcd, distances)
    clustered_pcds.sort(key=lambda pcd: len(pcd.points), reverse=True)

    # 如果只有一个聚类（极端情况），直接返回
    if len(clustered_pcds) < 2:
        logger.info("Only one distance cluster found, skipping boundary removal.")
        x_min, x_max, y_min, y_max = _get_min_max_xy(pcd2d)
        return pcd2d, _create_obb_from_bounds(x_min, x_max, y_min, y_max)

    # 外侧簇（边框）vs 内侧簇（内部）的点数
    max_points_num = len(clustered_pcds[0].points)
    min_points_num = len(clustered_pcds[1].points)

    # 如果相差不大则视为标定板没有边框，不用去除
    if max_points_num - min_points_num > min_points_num / 2:
